(function () {
  ...
  function toast(msg, type) {
    ...
    el.textContent = msg;
    el.style.cssText = 'position:fixed;bottom:24px;right:24px;background:#15181E;border:1px solid #1e293b;padding:12px 18px;border-radius:10px;font-size:13px;z-index:200';
  ...
  function showModal(html) {
    document.getElementById('modalRoot').innerHTML = '<div class="modal-bg" id="modalBg">' + html + '</div>';
  ...
  function renderKeys(keys) {
    ...
    if (!keys || !keys.length) {
      el.innerHTML = '<p style="color:var(--ax-muted);font-size:13px">No keys assigned yet</p>';
      return;
    }
    el.innerHTML = keys.map((k, i) => {
      const key = typeof k === 'string' ? k : k.key || k.license_key || '';
      return `<div class="key-row"><span>${key}</span><button class="btn btn-ghost btn-sm" data-copy-key="${i}">Copy</button></div>`;
    }).join('');
  ...
  function openUpgradeModal(current) {
    const close = showModal(`
      <div class="modal" style="max-width:400px">
        ...
        <div style="display:flex;flex-direction:column;gap:10px;margin:16px 0">
          <button class="btn btn-primary" data-tier="plus">Upgrade to Plus — $9/mo</button>
          <button class="btn btn-ghost" data-tier="enterprise">Contact for Enterprise</button>
        </div>
  ...
  async function loadDashboard() {
    ...
    if (profile.picture) {
      ...
      av.innerHTML = `<img src="${profile.picture}" alt="" style="width:40px;height:40px;border-radius:50%" />`;
    } else {
      document.getElementById('avatar').textContent = (profile.name || 'U')[0].toUpperCase();
    }

    const { ok, data } = await api('customer-api');
    if (!ok) { toast(data.error || 'Failed to load dashboard', 'error'); return; }

    const user = data.user || {};
    document.getElementById('subLabel').textContent = user.subscription_label || user.subscription || 'Free';
    ...
    if (selectedPlan && selectedPlan !== 'free') {
      ...
      setTimeout(() => openUpgradeModal(user.subscription_label || 'Free'), 500);
    }
  }
  ...
  document.getElementById('upgradeBtn').onclick = () => {
    openUpgradeModal(document.getElementById('subLabel').textContent);
  };
  ...
  document.getElementById('hwidResetBtn').onclick = async () => {
    const close = showModal(`
      <div class="modal" style="max-width:380px">
        ...
        <p style="color:var(--ax-muted);font-size:14px;margin:12px 0">This sends a reset request to the admin team. Approval may take up to 24 hours.</p>
    ...
    document.getElementById('confirmReset').onclick = async () => {
      const { ok, data } = await api('customer-api', {
        method: 'POST',
        body: JSON.stringify({ action: 'hwid_reset_request' }),
      });
      close();
